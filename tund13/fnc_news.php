		...
		while($stmt->fetch()){
			...
			if(isset($_SESSION["userid"])) {
				if($useridfromdb == $_SESSION["userid"]) {
					$temphtml.= ' | <a href="editnews.php?newsid=' .$newsidfromdb .'"> Redigeeri pilt</a></p>' ."\n";
				}
			}
			$temphtml .= "</p> \n";
			$temphtml .= "</div> \n";
...
function readSubmittedNewsPage($userid ,$limit, $page = 1){
	$newshtml = null;
	$skip = ($page - 1) * $limit;
	$conn = new mysqli($GLOBALS["serverhost"], $GLOBALS["serverusername"], $GLOBALS["serverpassword"], $GLOBALS["database"]);
	//$stmt = $conn->prepare("SELECT filename, alttext FROM vpphotos WHERE privacy >= ? AND deleted IS NULL ORDER BY vpphotos_id DESC LIMIT ?");
	...
	FROM vpnews
	JOIN vpusers ON vpnews.userid = vpusers.vpusers_id
	WHERE vpnews.userid = ? AND deleted IS NULL
	GROUP BY vpnews.vpnews_id DESC LIMIT ?, ?");
	echo $conn->error;
	$stmt->bind_param("iii", $userid, $skip, $limit);
	$stmt->bind_result($newsidfromdb, $useridfromdb, $firstnamefromdb, $lastnamefromdb, $filenamefromdb, $creationdate, $expirationdate, $titlefromdb, $contentfromdb);
	$stmt->execute();
	$temphtml = null;
	while($stmt->fetch()){
...
function updateNewsFetch($newsid){
	$conn = new mysqli($GLOBALS["serverhost"], $GLOBALS["serverusername"], $GLOBALS["serverpassword"], $GLOBALS["database"]);
	//$stmt = $conn->prepare("SELECT filename, alttext FROM vpphotos WHERE privacy >= ? AND deleted IS NULL ORDER BY vpphotos_id DESC LIMIT ?");
	//$stmt = $conn->prepare("SELECT vpphotos_id, filename, alttext FROM vpphotos WHERE privacy >= ? AND deleted IS NULL ORDER BY vpphotos_id DESC LIMIT ?,?");
	$stmt = $conn->prepare("SELECT vpusers.vpusers_id, vpnews.newsphoto, vpnews.title, vpnews.content, vpnews.vpnews_id
	FROM vpnews
	JOIN vpusers ON vpnews.userid = vpusers.vpusers_id
	WHERE vpnews.vpnews_id = ? AND deleted IS NULL");
	echo $conn->error;
	$stmt->bind_param("i", $newsid);
	$stmt->bind_result($useridfromdb, $filenamefromdb, $titlefromdb, $contentfromdb, $newsidfromdb);
	$stmt->execute();
	$stmt->fetch();
	$array = [$filenamefromdb, $titlefromdb, $contentfromdb, $newsidfromdb];
	$stmt->close();
	$conn->close();
	return $array;
}

function updateNews($newsid, $title, $newsinput, $mainphoto) {
	$notice = null;
	$conn = new mysqli($GLOBALS["serverhost"], $GLOBALS["serverusername"], $GLOBALS["serverpassword"], $GLOBALS["database"]);
	$stmt = $conn->prepare("SELECT userid FROM vpnews WHERE vpnews_id = ?");
	echo $conn->error;
	$stmt->bind_param("i", $newsid);
	...
	$stmt->fetch();
	$stmt->close();
	$mysql = "UPDATE vpnews SET title = ?, content = ?, userid = ? ";
	if($mainphoto != null) {
		$mysql .= "newsphoto = ? ";
	}
	$mysql .= "WHERE vpnews_id = ? AND deleted IS NULL";
	$stmt = $conn->prepare($mysql);
	$conn->set_charset("utf8");
	echo $conn->error;
	if($mainphoto != null) {
		$stmt->bind_param("ssisi", $title, $newsinput, $_SESSION["userid"], $mainphoto, $useridfromdb);
	} else {
		$stmt->bind_param("ssii", $title, $newsinput, $_SESSION["userid"], $useridfromdb);
	}
	var_dump($useridfromdb, $_SESSION["userid"]);
	if($useridfromdb == $_SESSION["userid"]) {
		if($stmt->execute()){
		...
		$notice = "Teil ei ole õigust uudist redigeerida! OMANIKU USERID: " .$useridfromdb ." TEIE USERID:" .$_SESSION["userid"] ." UUDISE ID:" .$newsid;
	}
	$stmt->close();
	$conn->close();
	return $notice;
