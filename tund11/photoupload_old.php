...
if(isset($_POST["photosubmit"])) {
	...
	if(empty($inputerror)) {
		...
		if($imagew > $photoMaxWidth or $imageh > $photoMaxHeight) {
			...
			//THUMBNAIL
			$thumbw = round(($imagew - $neww) / 2);
			$thumbh = round(($imageh - $newh) / 2);
			$mynewtempthumb = imagecreatetruecolor($thumbw, $thumbh);
			imagecopyresampled($mynewtempthumb, $mytempimage, 0, 0, 0, 0, $thumbw, $thumbh, $imagew, $imageh);
			$thumbtarget = $thumbnailuploaddir .$filename;
			saveImage($mynewtempthumb, $filetype, $thumbtarget);
		} else {
			$target = $photouploaddir_orig .$filename;
			$notice = saveImage($mytempimage, $filetype, $target);

			//THUMBNAIL
			$thumbw = round($imagew / 2);
			$thumbh = round($imageh / 2);
			$mynewtempthumb = imagecreatetruecolor($thumbw, $thumbh);
			imagecopyresampled($mynewtempthumb, $mytempimage, 0, 0, 0, 0, $thumbw, $thumbh, $imagew, $imageh);
			$thumbtarget = $thumbnailuploaddir .$filename;
			saveImage($mynewtempthumb, $filetype, $thumbtarget);
		...
		var_dump($mynewtempthumb, $mytempimage);
