...
function readPublicPhotoThumbsPage($privacy, $limit, $page = 1){
	...
	$stmt = $conn->prepare("SELECT filename, vpphotos_id, alttext FROM vpphotos WHERE privacy >= ? AND deleted IS NULL ORDER BY vpphotos_id DESC LIMIT ?,?");
	echo $conn->error;
	$stmt->bind_param("iii", $privacy, $skip, $limit);
	$stmt->bind_result($filenamefromdb, $idfromdb, $alttextfromdb);
	$stmt->execute();
	$temphtml = null;
	while($stmt->fetch()){
