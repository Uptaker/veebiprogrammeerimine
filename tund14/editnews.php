...
if(isset($_GET['newsid'])) {
  $newsid = $_GET['newsid'];
  $newsinfo = updateNewsFetch($newsid);
  $fnewsphoto = $newsinfo[0];
  $fnewstitle = $newsinfo[1];
  $fnewscontent = $newsinfo[2];
  $newsid = $newsinfo[2];
  $location = "Location: editnews.php?newsid=" .$newsid;
}
  ...
  if(isset($_POST["newssubmit"])) {
    ...
    //pilt
    if(file_exists($_FILES['photoinput']['tmp_name']) || is_uploaded_file($_FILES['photoinput']['tmp_name'])) {
      var_dump($_FILES["photoinput"]);
    ...
    if(empty($inputerror)) {
      ...
      if(empty($inputerror)) {
        if(file_exists($_FILES['photoinput']['tmp_name']) || is_uploaded_file($_FILES['photoinput']['tmp_name'])) {
          $notice = updateNews($newsid, $newstitle, $news, $filename);
          // header($location);
        } else {
          $notice = updateNews($newsid, $newstitle, $news, null);
          // header($location);
        $newstitle = null;
        $news = null;
        }
...
?>
  ...
  <p>Siin saate lisada uudist!</p>
  <p>NB! Uudis automaatselt aegub ühe aasta pärast!</p>
  ...
  <label for="newstitleinput">Uudise pealkiri</label><br>
	<input id="newstitleinput" name="newstitleinput" type="text" value="<?php echo $fnewstitle; ?>">
  <br><br>
  ...
  <br><br>
	...
	<textarea id="newsinput" name="newsinput" type="textarea"><?php echo $fnewscontent; ?></textarea>
	<br><br>
	<input type="submit" id="newssubmit" name="newssubmit" value="Salvesta uuendused">
  </form>
  <p id="notice">
  <?php
	echo $inputerror;
